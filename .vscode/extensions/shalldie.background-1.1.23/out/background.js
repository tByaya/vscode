...
const vscodePath_1 = __importDefault(require("./vscodePath"));
const getCss_1 = __importDefault(require("./getCss"));
...
class Background {
     ...
     */
    getCssContent() {
        return fs_1.default.readFileSync(vscodePath_1.default.cssPath, constants_1.ENCODE);
    }
    /**
     * 设置 css 文件内容
     *
    ...
    saveCssContent(content) {
        if (content && content.length) {
            fs_1.default.writeFileSync(vscodePath_1.default.cssPath, content, constants_1.ENCODE);
        }
    }
    /**
     * 初始化
    ...
    install(refresh) {
        ...
        // 自定义的样式内容
        const content = getCss_1.default(arr, config.style, config.styles, config.useFront).replace(/\s*$/, ''); // 去除末尾空白
        // 添加到原有样式(尝试删除旧样式)中
